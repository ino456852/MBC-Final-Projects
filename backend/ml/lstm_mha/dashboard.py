...
from models import CustomAttention, SumOverTime, MODELS_TO_TRAIN
from constant import TARGETS, MA_PERIODS, MODELS_DIR
...
def run_prediction_and_analysis():
    ...
    for target_name in TARGETS:
        ...
        _, _, X_test, test_dates, _, target_scaler = prepare_data_for_target(
            data_cpy, target_name
        )

        if final_df is None:
            final_df = pd.DataFrame(index=test_dates)

        y_true_values = data_cpy.loc[test_dates, target_name].values
        final_df[f"Actual_{target_name}"] = y_true_values

        for period in MA_PERIODS:
            data_cpy[f"{target_name}_MA{period}"] = (
                data_cpy[target_name].rolling(period).mean()
            )
            data_cpy[f"{target_name}_EMA{period}"] = (
                data_cpy[target_name].ewm(span=period, adjust=False).mean()
            )

        for col in data_cpy.columns:
            if "_MA" in col or "_EMA" in col:
                if target_name in col:
                    final_df[col] = data_cpy.loc[test_dates, col]

        target_metrics = {}
        for model_key in MODELS_TO_TRAIN.keys():
            model_path = MODELS_DIR / f"{target_name}_{model_key}_attention.keras"

            if not os.path.exists(model_path):
                final_df[f"Predicted_{model_key}_{target_name}"] = np.nan
                continue
            ...
            final_df[f"Predicted_{model_key}_{target_name}"] = pred_inversed
...
def get_prediction_chart():
    try:
        ...
        for i, target in enumerate(TARGETS, 1):
                ...
                if col_name in final_df and not final_df[col_name].isnull().all():
                    fig.add_trace(
                        go.Scatter(
                            ...
                            name=style["name"],
                            mode="lines",
                            line=dict(dash=style["dash"], color=style["color"]),
                            # legendgroup=legend_group_name,
                        ),
                        row=i,
                        col=1,
            ...
            for period in MA_PERIODS:
                ma_col = f"{target}_MA{period}"
                ema_col = f"{target}_EMA{period}"
                fig.add_trace(
                    go.Scatter(
                        x=final_df.index,
                        y=final_df[ma_col],
            ...
            if metrics:
                metrics_texts.append(
                    f"<b>{target.upper()} (MHA)</b> | MSE: {metrics['MSE']:.2f} | RMSE: {metrics['RMSE']:.2f} | MAE: {metrics['MAE']:.2f} | MAPE: {metrics['MAPE']:.2f}% | R²: {metrics['R2']:.4f}"
                )
        ...
        fig.update_layout(
            height=2200,
            title_text="통화별 환율 예측 결과 비교 (Optuna 최적화)",
            legend=dict(
                orientation="v",
                yanchor="top",
                y=1,
