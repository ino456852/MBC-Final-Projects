...
from .constatnt import TARGETS, PRED_CSV_PTH
...
# 원본 시각화 함수들 (기능 유지)
# ------------------------------------------------
def load_2024_predictions():
    """2024년 예측 결과 로드"""
    try:
        return pd.read_csv(PRED_CSV_PTH, index_col=0, parse_dates=True)
    except FileNotFoundError:
        print(f"'{PRED_CSV_PTH}' 파일을 찾을 수 없습니다.")
        return None


def create_2024_prediction_chart():
    """2024년 실제값 vs 예측값 비교 차트"""
    predictions_df = load_2024_predictions()
    ...
    fig = make_subplots(
        ...
        cols=1,
        subplot_titles=[
            f"{t.upper()} 환율 실제값 vs 예측값 (2024년~)" for t in TARGETS
        ],
    )
    ...
    fig.update_layout(
        title="2024년 환율 실제값 vs 예측값 비교", height=1500, hovermode="x unified"
    )
    return fig
...
def create_performance_metrics_table():
    ...
    fig.update_layout(title="2024년 예측 성능 지표", height=300)
...
def create_correlation_scatter_plots():
    ...
    fig.update_layout(title="실제값 vs 예측값 산점도 (2024년~)", height=800)
...
async def chart() -> HTMLResponse:
    """2024년 예측 차트 표시"""
    try:
        fig = create_2024_prediction_chart()
        if fig:
            return HTMLResponse(fig.to_html(full_html=True, include_plotlyjs="cdn"))
        else:
            return HTMLResponse(
                f"<h1>데이터를 찾을 수 없습니다</h1><p>{PRED_CSV_PTH} 파일이 필요합니다.</p>",
                status_code=404,
            )
    except Exception as e:
...
async def scatter() -> HTMLResponse:
    """산점도 표시"""
    try:
        fig = create_correlation_scatter_plots()
        if fig:
            return HTMLResponse(fig.to_html(full_html=True, include_plotlyjs="cdn"))
        else:
            return HTMLResponse(
...
async def analysis() -> HTMLResponse:
    """전체 분석 결과"""
    predictions_df = load_2024_predictions()
    if predictions_df is None:
    ...
    # 차트들 생성
    chart_fig = create_2024_prediction_chart()
    table_fig = create_performance_metrics_table()
    scatter_fig = create_correlation_scatter_plots()

    html_content = "<h1>2024년 환율 예측 분석 결과</h1>"

    if chart_fig:
        html_content += "<h2>예측 차트</h2>" + chart_fig.to_html(
            full_html=False, include_plotlyjs="cdn"
        )

    if table_fig:
        html_content += "<h2>성능 지표</h2>" + table_fig.to_html(
            full_html=False, include_plotlyjs=False
        )

    if scatter_fig:
        html_content += "<h2>산점도 분석</h2>" + scatter_fig.to_html(
            full_html=False, include_plotlyjs=False
        )
...
# 간단한 표시 함수들 (원본 기능 유지)
def show_2024_prediction_chart():
    """2024년 예측 차트 표시"""
    fig = create_2024_prediction_chart()
    if fig:
        fig.show()
...
def show_all_2024_visualizations():
    """모든 2024년 예측 시각화 표시"""
    predictions_df = load_2024_predictions()
    if predictions_df is None:
        return
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--api-only":
        ...
        try:
            ...
            print(f"❌ 서버 시작 실패: {e}")
            print(
                "💡 다른 포트로 시도해보세요: uvicorn predict_visualization2:app --port 8003"
            )
