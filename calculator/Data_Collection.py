...
df_usd = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "USD"}
)
...
data = response.json()
rows = data["StatisticSearch"]["row"]
df_gbp = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "GBP"}
)
...
data = response.json()
rows = data["StatisticSearch"]["row"]
df_jpy = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "JPY(100)"}
)
...
data = response.json()
rows = data["StatisticSearch"]["row"]
df_eur = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "EUR"}
)
...
data = response.json()
rows = data["StatisticSearch"]["row"]
df_cny = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "CNY"}
)
...
data = response.json()
rows = data["StatisticSearch"]["row"]
df_cny_usd = pd.DataFrame(rows)[["TIME", "DATA_VALUE"]].rename(
    columns={"TIME": "DATE", "DATA_VALUE": "CNY_USD"}
)
...
# 파일 병합
df_merged = (
    df_usd.merge(df_jpy, on="DATE", how="outer")
