...
def objective(trial, target_currency):
    print(f"\n===== Trial #{trial.number} ({target_currency.upper()}) 시작 =====")

    model_name = trial.suggest_categorical('model_name', ['custom', 'mha', 'dot', 'cross', 'causal', 'self'])
    build_fn = MODELS_TO_TRAIN[model_name]
    optimizer_name = trial.suggest_categorical('optimizer_name', ['AdamW', 'Nadam', 'Adadelta', 'Adafactor'])
    ...
    if model_name == 'cross':
        trial_config.LSTM_UNITS_HALF = trial.suggest_int('LSTM_UNITS_HALF', 25, 125, step=25)

    X_train, y_train, _, _, _, _ = prepare_data_for_target(master_data, target_currency)
    num_features = X_train.shape[2]
    model = build_fn(config.LOOK_BACK, num_features, params=trial_config)

    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=config.PATIENCE, restore_best_weights=True)
    history = model.fit(
        X_train, y_train, epochs=config.EPOCHS, batch_size=config.BATCH_SIZE,
        validation_split=config.VALIDATION_SPLIT, callbacks=[early_stopping], verbose=0
    )

    val_loss = np.min(history.history['val_loss'])
    print(f"Trial #{trial.number} ({target_currency.upper()}) 완료. 모델: {model_name.upper()}, 옵티마이저: {optimizer_name}, Val Loss: {val_loss:.6f}")
...
if __name__ == '__main__':
    ...
    all_best_params = {}
    n_trials_per_currency = 100

    for currency in TARGETS:
        print(f"\n\n{'='*20} {currency.upper()} 통화 튜닝 시작 {'='*20}")
        study = optuna.create_study(study_name=f"{currency}_study", direction='minimize')

        try:
            study.optimize(partial(objective, target_currency=currency), n_trials=n_trials_per_currency)
