...
def get_optimizer(name, learning_rate):
    if name == 'AdamW': return tf.keras.optimizers.AdamW(learning_rate)
    if name == 'Nadam': return tf.keras.optimizers.Nadam(learning_rate)
    if name == 'Adadelta': return tf.keras.optimizers.Adadelta(learning_rate=1.0)
    if name == 'Adafactor': return tf.keras.optimizers.Adafactor()
...
def finalize_model(inputs, attention_output, params):
    ...
    optimizer = get_optimizer(getattr(params, 'optimizer_name', 'Adam'), params.LEARNING_RATE)
...
class CustomAttention(tf.keras.layers.Layer):
    def __init__(self, **kwargs): super().__init__(**kwargs); self.dense1, self.dense2 = None, None
    def build(self, input_shape):
        self.dense1 = tf.keras.layers.Dense(input_shape[-1], activation='tanh')
        self.dense2 = tf.keras.layers.Dense(1)
        super().build(input_shape)
    def call(self, x):
        uit = self.dense1(x); ait = self.dense2(uit)
...
def build_cross_attention_model(look_back, num_features, params):
    ...
    base_lstm_out = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(params.LSTM_UNITS, return_sequences=True))(base_features_input)
    target_lstm_out = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(params.LSTM_UNITS_HALF, return_sequences=True))(target_related_input)
...
MODELS_TO_TRAIN = {
    "custom": build_custom_attention_model, "mha": build_multi_head_attention_model,
    "dot": build_dot_product_attention_model, "cross": build_cross_attention_model,
    "causal": build_causal_self_attention_model, "self": build_self_attention_model
}